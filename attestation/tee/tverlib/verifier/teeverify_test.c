#include "teeverifier.c"
uint8_t certTem[] = {
0x01, 0x00, 0x00, 0x00, // ak_cert_version
0x01, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, // ak_cert_ts
0x70, 0x72, 0x6f, 0x76, 0x69, 0x73, 0x69, 0x6f, 0x6e, 0x20, 0x6e, 0x6f,
0x20, 0x61, 0x73, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, // ak_cert_tag
0x08, 0x00, 0x00, 0x00, // ak_cert_param_count
0x00, 0x00, 0x00, 0x10, 0x01, 0x00, 0x02, 0x00, 
0x00, 0x00, 0x00, 0x00, // ak_cert_alg_sign
0x01, 0x00, 0x00, 0x10, 0x02, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00,  // ak_cert_alg_hash
0x00, 0x00, 0x00, 0x20, 0x20, 0x00, 0x00, 0x00, 0x90, 0x00, 0x00, 0x00, //ak_cert_RA_TAG_QTA_IMG_HASH
0x02, 0x00, 0x00, 0x20,
0x20, 0x00, 0x00, 0x00, 0xb0, 0x00, 0x00, 0x00, //ak_cert_RA_TAG_QTA_MEM_HASH
0x04, 0x00, 0x00, 0x20, 0x20, 0x00, 0x00, 0x00, 
0xd0, 0x00, 0x00, 0x00, //ak_cert_RA_TAG_RESERVED
0x05, 0x00, 0x00, 0x20, 0x00, 0x02, 0x00, 0x00, 0xf0, 0x00, 0x00, 0x00, //ak_cert_RA_TAG_AK_PUB
0x06, 0x00, 0x00, 0x20, 0x00, 0x02, 0x00, 0x00, 0xf0, 0x02, 0x00, 0x00, //ak_cert_RA_TAG_SIGN_DRK
0x08, 0x00, 0x00, 0x20,
0x20, 0x06, 0x00, 0x00, 0xf0, 0x04, 0x00, 0x00, //ak_cert_RA_TAG_CERT_DRK
0x46, 0x6a, 0x3a, 0x9a, 0xc3, 0x06, 0x1d, 0xae,
0xe5, 0x71, 0xf9, 0xaa, 0x95, 0x8b, 0x2d, 0x4b, 0xc4, 0xfb, 0xec, 0x7c, 0x96, 0xd4, 0xb5, 0x26,
0x1f, 0xf4, 0x6a, 0xb5, 0xb1, 0x41, 0x80, 0xb6, //ak_cert_QTA_IMG_HASH
0x80, 0x81, 0xb0, 0xb1, 0x76, 0xdc, 0x0b, 0x8c,
0xa3, 0x4d, 0xe8, 0x3b, 0xbf, 0x10, 0xf4, 0xb4, 0x1a, 0xd8, 0xdf, 0xdf, 0x76, 0xd7, 0x4a, 0xf7,
0xbd, 0x9c, 0x5a, 0xe7, 0xde, 0x71, 0xfc, 0xf9, //ak_cert_QTA_mem_HASH
0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, //ak_cert_reserved
, // ak_cert_akpub
, // ak_cert_sign_drk
, //drk֤��
};
void TestNoDAA(buffer_data *report){
    buffer_data akcert,signak,signdata;
    uint32_t scenario;
    bool rt = getDataFromReport(report,&akcert,&signak,&signdata,&scenario);
    if(!rt){
        printf("test get data from report failed\n");
        return;
    }
    printf("test get data from report succeeded\n");
    //update the akcert in report
    akcert.size=sizeof(certTem);
    memcpy(akcert.buf,certTem,akcert.size);
    //get drk cert and other data from akcert
    buffer_data signdataDrk,signdrk,certdrk,akpub;
    rt = getDataFromAkCert(&akcert,&signdataDrk,&signdrk,&certdrk,&akpub);
    if(!rt){
        printf("test get data from akcert failed\n");
        return;
    }
    printf("test get data from akcert succeeded\n"); 

    //verify the drk cert
    rt = verifysig(&signdataDrk,&signdrk,&certdrk,scenario);  
    if(!rt){
        printf("test verify drk cert failed\n");
        return;
    }
    printf("test verify drk cert succeeded\n");
    //verify the akcert
    uint8_t digest[SHA256_DIGEST_LENGTH];
    _SHA256(signdata.buf, signdata.size, digest);

    // step 3 : perform signature verification
    buffer_data mhash = {sizeof(digest), digest};
    dumpDrkCert(&akpub);
    BIO *bufio;
    EVP_PKEY *pubKey = NULL;
    pubKey = buildPubKeyFromModulus(&akpub);
    if (pubKey==NULL){
        printf("Error getting public key from certificate\n");
    }
    rt = verifySigByKey(&mhash, &signak, pubKey);  
    if(!rt){
        printf("test verify ak cert failed\n");
        return;
    }
    printf("test verify ak cert succeeded\n"); 
    return;
}

int main()
{
    // test getDrkPubFromCertDrk
    EVP_PKEY *key;
    // bool rt = TestgetDrkPubFromCertDrk(key);
    // if(!rt){
    //     printf("getDrkPubFromCertDrk is failed!\n");
    // }
    int data_len = 4888;//3624
    uint8_t report[] = {
    , };
    uint8_t report2[] = {
, };
    
    buffer_data bufdata;
    bufdata.buf = report2;
    bufdata.size = data_len;
    TestNoDAA(&bufdata);
    // buffer_data test;
    // bool tv = tee_verify_signature(&bufdata);
    // if (tv)
    // {
    //     printf("verify signature succeeded\n");
    // }
    // else
    // {
    //     printf("verify signature failed\n");
    // }

    // buffer_data nonce;
    // nonce.size = 9;
    // nonce.buf = "challenge";
    // TA_report *report1 = Convert(&bufdata);
    // printf("nonce:%s\n", report1->scenario);
    // test_print(report1->uuid,UUID_SIZE,"uuid");
    // printf("scenario:%u\n", report1->scenario);
    // test_print(report1->image_hash,HASH_SIZE,"img_hash");
    // test_print(report1->hash,HASH_SIZE,"hash");
    // test_print(report1->reserve,HASH_SIZE,"reserve");
    // tv = tee_verify_nonce(&bufdata,&nonce);
    // if (tv)
    // {
    //     printf("verify nonce succeeded\n");
    // }
    // else
    // {
    //     printf("verify nonce failed\n");
    // }
    // tv = tee_verify(&bufdata, 3, "basevalue.txt");//test report1-success
    // if (tv)
    // {
    //     printf("verify succeeded\n");
    // }
    // else
    // {
    //     printf("verify failed!\n");
    // }
    return 0;
}