...
func createQcaConfigFile() {
	ioutil.WriteFile(ConfigPath, []byte(QcaConfig), TEST_FILE_MODE)
}

func createAttesterConfigFile() {
	ioutil.WriteFile(ConfigPath, []byte(attesterCfg), TEST_FILE_MODE)
}

func createBaseValueFile() {
	ioutil.WriteFile(basevaluePath, []byte(basevalueCont), TEST_FILE_MODE)
}
