...
func TestDoGetTeeReport(t *testing.T) {
	createTasConfigFile()
	createFiles()
	config.LoadConfigs()
	tasServer := config.GetServerPort()

	go server.StartServer(tasServer)//tas
	...
	_ , err = Makesock(server)
	if err != nil {
		t.Error(err)
	}

	req2 := &qapi.GetReportRequest{
		Uuid:    id2,
		Nonce:   nonce2,
		WithTcb: tcb,
	}
	...
	qapiserver.StopServer()

	qcatools.SetScenario(RA_SCENARIO_AS_NO_DAA)
	go qapiserver.StartServer()
	req3 := &qapi.GetReportRequest{
		Uuid:    id3,
		Nonce:   nonce3,
		WithTcb: tcb,
	}
