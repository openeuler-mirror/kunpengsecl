...
COUNT_RAS=$(grep 'going to call DeleteKey()' ${DST}/ras/echo.txt |wc -l)
echo "count ras: ${COUNT_RAS}" | tee -a ${DST}/control.txt
### list the log
### tail -f ${DST}/rac-1/echo.txt
COUNT_DEMO_CA=$(grep 'ta exit succeeded' ${DST}/demo_ca/echo.txt |wc -l)
echo "count demo ca: ${COUNT_DEMO_CA}" | tee -a ${DST}/control.txt

if (( ${COUNT_RAS} == 1 && ${COUNT_DEMO_CA} == 1))
then
    echo "key cache cleanup succeeded"
    echo "test succeeded!"
    exit 0
else
    echo "key cache cleanup failed"
    echo "test failed!"
    exit 1
fi
