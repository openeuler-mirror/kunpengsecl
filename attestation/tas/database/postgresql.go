...
func RegisterClientByDC(dvcert []byte, clientinfo string) (DKeyRow, error) {
	var d DKeyRow = DKeyRow{
		Device_cert: string(dvcert),
	}
	if db == nil {
		return DKeyRow{}, errors.New("can not find database")
	}
	err := db.QueryRow(sqlFindDeviceByDC, d.Device_cert).Scan(&d.Registered)
	if err != nil {
		return DKeyRow{}, err
	}
	if d.Registered {
		return DKeyRow{}, errors.New("device has been registered")
	}
	d = DKeyRow{
		Registered:  true,
		Reg_time:    time.Now(),
	...
	_, err = db.Exec(sqlRegisterClientByDC,
		d.Registered, d.Reg_time, d.Client_info, d.Registered)
	if err != nil {
		return DKeyRow{}, err
	}
	return d, nil
...
func InsertAKCertRow(akcert []byte, did int64) error {
	...
	err := db.QueryRow(sqlFindAKCertByAC, a.Akey_cert).Scan(&a.Id) // 可有可无
	if err == nil {
